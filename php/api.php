...
class APIRequest {
	...
	function execute() {
		try {
			...
			echo "Error from php api: " . $e->getMessage() . '<br>';
		}
	...
	/**
	 * @from Crypt_RSA
	 * Creates the Authorisation bearer token using the RSA public key provided from Crypt_RSA
	 * 
	 */ 
   ...
   // 3rd trial with OpenSSl 
	function create_bearer_token() {
		$publicKey = $this->context->get_public_key();
		$api_key = $this->context->get_api_key();
		
		// Convert public key from PEM to resource
		...
		$encrypted = '';
		if (openssl_public_encrypt($api_key, $encrypted, $publicKeyResource)) {
			$api_encrypted = base64_encode($encrypted);
			return $api_encrypted;
		} else {
